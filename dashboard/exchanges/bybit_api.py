...
def get_bybit_tickers():
    try:
        ...
        for item in data:
            ...
            ret_dict['price'].append(float(item['lastPrice']))
            ret_dict['volume'].append(item['volume24h'])
...
class BybitExchange(BaseExchange):
    ...
    def get_24h_volume(self) -> Dict[str, float]:
        ...
        try:
            ...
            r.raise_for_status()
            data = r.json().get("result", {}).get("list", [])
            return {item['symbol']: float(item['volume24h']) * prices.get(item['symbol'], 0) for item in data}
        except Exception as e:
            st.error(f"❌ Bybit 거래량 정보를 불러올 수 없습니다: {e}")
            return {}
