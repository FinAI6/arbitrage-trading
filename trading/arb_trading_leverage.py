...
def convert_symbol(exchange, raw_symbol):
    try:
        # Bybit v5: 선물 마켓 명시
        exchange.load_markets(params={'category': 'linear'} if exchange.id == 'bybit' else {})

        formatted_input = raw_symbol.replace('/', '').replace(':', '').upper()

        for market_id, market in exchange.markets.items():
            formatted_market = market['id'].replace('/', '').replace(':', '').upper()

            if formatted_input == formatted_market:
                # Bybit의 경우 linear인지 확인
...
def safe_set_leverage(exchange, symbol, leverage):
    try:
        # Bybit의 경우 선물 마켓을 명시적으로 로드
        markets = exchange.load_markets(params={'category': 'linear'} if exchange.id == 'bybit' else {})

        # 심볼 표준화: ZKJUSDT, ZKJ/USDT 등 → ccxt가 이해할 수 있는 심볼 찾기
        ...
        if not normalized_symbol:
            print(f"❌ {exchange.id} 마켓에서 유효한 심볼을 찾을 수 없음: {symbol}")
            return

        # 카테고리 파라미터 설정 (Bybit 전용)
        params = {'category': 'linear'} if exchange.id == 'bybit' else {}

        print(f"🎯 레버리지 설정 시도: {exchange.id} | 심볼: {normalized_symbol}")

        exchange.set_leverage(leverage, normalized_symbol, params=params)

        print(f"✅ 레버리지 {leverage}배 설정 완료 ({exchange.id}, {normalized_symbol})")

    except Exception as e:
        print(f"❌ 레버리지 설정 실패 ({exchange.id}, {symbol}): {e}")
